...
- **รูปบัตรประชาชนบังคับแนบ** ทั้งฟอร์มสาธารณะ (`/request`) และหน้าลงทะเบียนของเจ้าหน้าที่
  ...
  หรือมีไฟล์อยู่จะแนบไม่ได้ · ใช้ `components/PhotoInput.tsx` ตัวเดียวกันทั้งสองฟอร์ม
- **รูปภาพ**: รูปบัตร/รูปอาการเป็นข้อมูลสุขภาพ เสิร์ฟผ่าน `/api/files/[...id]` ที่เช็ค session เท่านั้น
  ...
  · **ไฟล์ใน Blob เป็น `access: 'private'` ทั้งหมด** รวมรูปอุปกรณ์ด้วย — เบราว์เซอร์ไม่เคยเห็น
  URL ของ Blob อยู่แล้ว ทุกอย่างผ่าน route ของเราที่อ่านฝั่ง server แล้ว stream ออกไป
  · `readUpload()` ลองอ่านแบบ private ก่อนแล้วค่อย public เพราะไฟล์เก่าที่อัปก่อนหน้านี้
  ถูกสร้างเป็น public ไปแล้ว และ access level ของ blob แก้ทีหลังไม่ได้
  · **`/api/equipment-photo` ต้องมีข้อยกเว้น Cache-Control ใน `next.config.mjs` ด้วย**
  ไม่งั้น `private, no-store` ที่ครอบ `/api/*` ไว้จะทับ header ของ route เอง
  แล้วรูปหน้าแรกจะถูกดึงผ่าน function ใหม่ทุกครั้งที่มีคนเปิดเว็บ
- **รูปอุปกรณ์บนหน้าแรกมาจากที่เจ้าหน้าที่อัปโหลดเท่านั้น** ไม่มีรูปสำเร็จรูปใน `public/assets`
  ให้ fallback อีกแล้ว (เดิมเดาจากชื่ออุปกรณ์ด้วย regex ซึ่งอุปกรณ์ชื่อใหม่จะไม่เข้าเงื่อนไข
  แล้วเงียบ ๆ ไม่มีรูป ส่วนตัวที่เข้าเงื่อนไขก็โชว์รูปสำเร็จรูปที่อาจไม่ตรงของจริง)
  · อุปกรณ์ที่ยังไม่มีรูป ขึ้นไอคอนกล่องแทน
...
- **ลิสต์ทุกอันในหน้าเจ้าหน้าที่ต้องบอกจำนวน + โหลดทีละ 20** ผ่าน `useInfiniteList()`
  ใน `components/staff/InfiniteList.tsx` · `<ListCount>` อยู่เหนือแถวแรก `<ListMore>` อยู่ท้ายลิสต์
  (เป็นตัวที่ IntersectionObserver จับ เลื่อนถึงแล้วดึงหน้าถัดไป)
  · ฝั่ง API รับ `?limit=&offset=` และคืน `total` มาด้วย · **ไม่ส่ง `limit` = คืนทั้งหมด**
  เพราะ dropdown เลือกอุปกรณ์ หน้าแรกสาธารณะ และ dashboard ต้องการครบทุกแถว
  · ลิสต์ที่กรอง/เรียงในเบราว์เซอร์ (สต็อก, เจ้าหน้าที่ — ข้อมูลน้อย) ใช้ `useArrayPage()` แทน
  แต่หน้าตาเหมือนกันทุกประการ
  · **`ORDER BY` ของ endpoint ที่แบ่งหน้า ต้องมี id ต่อท้ายเสมอ** เช่น `at DESC, log_id DESC`
  — แถวที่เขียนพร้อมกันใน statement เดียวมี timestamp เท่ากันเป๊ะ พอ sort ไม่ unique
  แล้ว LIMIT/OFFSET จะส่งแถวเดิมซ้ำในหน้าถัดไปและทำอีกแถวหาย (เจอมาแล้วจริง ๆ)
  · สถิติที่ต้องคิดจาก "ทุกแถว" (เช่น % คืนตรงเวลา) ต้องคิดใน SQL ไม่ใช่จาก array ที่โหลดมา
  ไม่งั้นตัวเลขจะขยับตอนเลื่อนดู — ตอนนี้อยู่ที่ `GET /api/borrowers/[id]` (`on_time`)
